...
from py.config import Proxy,UrlMain,UrlLast,HeadersMain,HeadersLast,Payload
...
def checker(record):
    ...
    while not done:
        try:
            ...
            UrlLastFormated = UrlLast.format(Email=email,TimeStamp13=TimeStamp)

            r = s.get(UrlLastFormated,headers = HeadersLast, timeout=5)

            #log.add_records_to_logs(str(r.json()).replace("\n","").replace("  ","").split('<div class="modalErrors">\\n \\n')[1].split(".")[0])
            if "to reset your password have been emailed to you" in str(r.json()):
                log.add_records_to_logs("Registered")
            s.close()
            done = True
            count+=1
            if count%50 == 0:
...
def main(start_from):
    ...
    log = logs_handler("Logs/21Century")
    csv_handler.create("21Century")
